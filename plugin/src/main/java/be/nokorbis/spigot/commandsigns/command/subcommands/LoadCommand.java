...
public class LoadCommand extends CommandRequiringManager {
	...
	public LoadCommand(NCommandSignsManager manager) {
		super(manager, "reload", new String[] {"load"});
			...
			try {
				...
				if (old != null) {
					if (old.getId() != id) {
						...
						msg = msg.replace("{WORLD}", loc.getWorld().getName()).replace("{X}", String.valueOf(loc.getBlockX())).replace("{Y}", String.valueOf(loc.getBlockY())).replace("{Z}", String.valueOf(loc.getBlockZ()));
